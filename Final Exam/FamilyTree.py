...
class Family(object):
    ...
    def cousin(self, a, b):
        ...
        self.a = a
        self.b = b
        node1 = self.names_to_nodes[self.a]
        node2 = self.names_to_nodes[self.b]
        self.cousin_type = 0
        
        # code to get degrees removed
        node1_to_founding_ancestor = 0
        node2_to_founding_ancestor = 0
        parentnode1 = node1.get_parent()
        parentnode2 = node2.get_parent()
        while parentnode1 != None:
            node1_to_founding_ancestor += 1
            if parentnode1 == node2:
                self.cousin_type = -1
            parentnode1 = parentnode1.get_parent()
        while parentnode2 != None:
            node2_to_founding_ancestor += 1
            if parentnode2 == node1:
                self.cousin_type = -1
            parentnode2 = parentnode2.get_parent()
        self.degrees_removed = abs(node1_to_founding_ancestor - node2_to_founding_ancestor)
        
        # code to get cousin type
        if self.cousin_type == -1:
            pass
        elif node1 == node2:
            self.cousin_type = -1
        else:
            self.cousin_type = 0           
            diff = node1_to_founding_ancestor - node2_to_founding_ancestor
            if diff > 0:
                parentnode1 = node1
                for i in range(diff):
                    parentnode1 = parentnode1.get_parent()
                ...
                while parentnode1 != parentnode2:
                    ...
                    parentnode2 = parentnode2.get_parent()
        # return cousin tuple
        return (self.cousin_type, self.degrees_removed)
